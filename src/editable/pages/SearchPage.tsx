...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white/6 shadow-[0_20px_50px_rgba(0,0,0,0.18)] backdrop-blur-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-black">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="rounded-full bg-white/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--slot4-page-text)]">{taskLabel}</span> : null}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-panel-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="editable-dark-panel grid gap-8 rounded-[2.5rem] p-6 md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-[var(--editable-border)] bg-white/4 p-4 sm:p-5">
              <input type="hidden" name="master" value="1" />
              <label className="editable-dark-input flex items-center gap-3 rounded-2xl px-4 py-3">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="editable-dark-input flex items-center gap-2 rounded-2xl px-4 py-3">
                ...
                <select name="task" defaultValue={task} className="editable-dark-input rounded-2xl px-4 py-3 text-sm font-black outline-none">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--editable-cta-bg)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-cta-text)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/article" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white/5 px-5 py-3 text-sm font-black">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/4 p-10 text-center">
