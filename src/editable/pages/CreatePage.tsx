...
const fieldClass = 'editable-dark-input rounded-2xl px-4 py-3 text-sm font-bold text-[var(--slot4-page-text)] outline-none transition focus:border-[var(--slot4-accent)]'
    ...
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[var(--slot4-panel-bg)] px-4 py-16 text-[var(--slot4-page-text)] sm:px-6 lg:px-8">
          <section className="editable-dark-panel mx-auto grid max-w-5xl gap-8 rounded-[2.8rem] p-7 md:grid-cols-[0.9fr_1.1fr] md:p-10">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white/5 px-6 py-3 text-sm font-black">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-panel-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="editable-dark-panel grid gap-8 rounded-[2.8rem] p-6 lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-current bg-[var(--editable-cta-bg)] text-[var(--editable-cta-text)]' : 'border-[var(--editable-border)] bg-white/5 hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-[var(--editable-border)] bg-white/4 p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-white/8 px-4 py-2 text-xs font-black uppercase tracking-[0.16em]">{session.name}</span>
              </div>
              ...
              {created ? (
                <div className="mt-5 rounded-2xl border border-emerald-400/25 bg-emerald-500/12 p-4 text-emerald-200">
