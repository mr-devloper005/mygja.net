...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-16 border-t border-[var(--tk-line)] pt-10">
      ...
      <form onSubmit={submit} className="mt-6 rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-white/70 p-5 shadow-[0_20px_50px_rgba(17,24,39,0.06)] backdrop-blur-sm">
        <input
          value={name}
        ...
        <textarea
          ...
          rows={3}
          maxLength={1500}
          className="mt-3 w-full resize-y rounded-[1.4rem] border border-[var(--tk-line)] bg-white px-4 py-3 text-sm leading-6 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--tk-accent)] px-6 py-2.5 text-sm font-bold text-[var(--tk-on-accent)] transition hover:-translate-y-0.5 hover:brightness-95 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div className="mt-6 grid gap-3">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-white/70 p-5 shadow-[0_20px_50px_rgba(17,24,39,0.06)] backdrop-blur-sm">
