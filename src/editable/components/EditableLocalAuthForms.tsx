...
const inputClass = 'editable-dark-input h-12 w-full rounded-full px-4 text-sm font-medium outline-none transition focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--editable-cta-bg)] px-6 text-sm font-bold text-[var(--editable-cta-text)] transition hover:-translate-y-0.5 hover:brightness-95 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.2rem] px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-emerald-500/14 text-emerald-200' : 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-page-text)]'}`}>{message}</p> : null}
